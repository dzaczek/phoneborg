...
func TestModelAndPlacementCommands(t *testing.T) {
	...
	expect(run("models", "recommend", id, "m,l"), "recommended")
	expect(run("classes"), "CLASS", "xs", "10 GiB+", id)
	expect(run("models", "default", id), "default model is "+id)
	expect(run("models"), id+" (default)", "coding")
	expect(run("placement"), "default model: "+id, "NODE", "n1", "default")
	...
	expect(run("placement", "set", id, "percent=50", "classes=s,l"), "percent", "50%", "s,l")
	expect(run("placement", "-json"), `"mode": "percent"`)
