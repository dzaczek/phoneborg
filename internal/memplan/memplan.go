...
func NeedBytes(fileSize int64, ctx, slots int, shape Shape, kvType string) int64 {
...
// candidate, then ctx halved down to MinCtx, then 1 slot.
func Plan(fileSize int64, shape Shape, ctx, slots int, kvType string, budget int64) Result {
	if ctx <= 0 {
		ctx = MinCtx
	}
	if slots <= 0 {
	...
	try := func(ctx, slots int) (string, int64, bool) {
		for _, kv := range kvs {
			need := NeedBytes(fileSize, ctx, slots, shape, kv)
			if need <= budget {
				return kv, need, true
			}
		}
	...
	return Result{Ctx: ctx, Slots: slots, KV: kvs[len(kvs)-1], Need: NeedBytes(fileSize, ctx, slots, shape, kvs[len(kvs)-1])}
}
